...
The Ring plugin in this repository uses [`Recoveredd/ring`](https://github.com/Recoveredd/ring), which adapts the Ring API v3 location, device, event, and permission handling from [`dgreif/ring#1749`](https://github.com/dgreif/ring/pull/1749) while preserving Scrypted's streaming changes. The custom plugin version is identified by the `-recoveredd` suffix and is based on the next patch number so Scrypted does not mistake the current upstream release for an update. A future upstream release with that patch number remains newer than the prerelease. This fork can return to the upstream Ring submodule after equivalent shared-account support is released and validated.
